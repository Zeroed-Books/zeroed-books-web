...
const LoginPage = () => {
    ...
    const loginMutation = useMutation<void, AxiosError, Credentials>(createCookieSession, {
        onSuccess: () => {
    ...
    return (
        <Container size="xs">
            ...
            {loginMutation.isError && <Alert icon={<CrossCircledIcon />} title="Error!" color="red" mb="lg">
                {loginMutation.error.response?.data?.message ?? "Unknown"}
            </Alert>}
            <LoginForm loading={loginMutation.isLoading} onSubmit={loginMutation.mutate} />
        </Container>
    );
};
